...
{ 
	...
	public function canChangeMaterialComp($app_id,$audit_id=''){
		...
		//$Audit = null;
		if($audit_id ==''){
			$Audit = Audit::find()->where(['app_id' => $app_id])->one();
		}else{
			$Audit = Audit::find()->where(['id' => $audit_id])->one();
		}
		$applicationNotInStatus = [
			$model->arrEnumStatus['pending_with_customer'],
			$model->arrEnumStatus['failed'],
			$model->arrEnumStatus['osp_reject']
		];

		if($model->status > $model->arrEnumStatus['submitted'] && !in_array($model->status, $applicationNotInStatus) 
		&& ($model->audit_type == $model->arrEnumAuditType['normal'] 
			|| $model->audit_type == $model->arrEnumAuditType['standard_addition'] 
			|| $model->audit_type == $model->arrEnumAuditType['renewal']) 
		
		){
			//&& ($Audit === null || $Audit->auditplan === null || ($Audit->auditplan->status > $Audit->auditplan->arrEnumStatus['waiting_for_review']) )
			if($resource_access ==1 
			|| ($user_type == 1 && 
				(in_array('manage_product_material_composition',$rules) || in_array('audit_review',$rules) || in_array('audit_execution',$rules))) 
			...
			|| ($user_type == 3) 
				...
				if($user_type == 3){
					...
					}else if($resource_access == '5' && $model->franchise_id == $franchiseid && $is_headquarters!=1){
						$canChangeMaterialComp = 1;
					}else if($model->franchise_id == $userid && $is_headquarters!=1){
						$canChangeMaterialComp = 1;
					}
					
					if($Audit!==null && $Audit->status > $Audit->arrEnumStatus['submitted']){
						$canChangeMaterialComp = 0;
					}
				}else if($user_type == 2){
					$canChangeMaterialComp = 1;
				...
				}else if($resource_access ==1 ){
					$canChangeMaterialComp = 1;
				}else if($user_type == 1 && $Audit !== null && $Audit->auditplan !== null){
					if($Audit->status > $Audit->arrEnumStatus['submitted'] && $Audit->status < $Audit->arrEnumStatus['finalized'] 
					&&  $Audit->auditplan->application_lead_auditor == $userid && Yii::$app->userrole->hasRights(['audit_execution'])){
						$canChangeMaterialComp = 1;	
					}else if($Audit->status > $Audit->arrEnumStatus['followup_submitted']   
					&&  $Audit->auditplan->followup_application_lead_auditor == $userid && Yii::$app->userrole->hasRights(['audit_execution'])){
						$canChangeMaterialComp = 1;	
					}
					if( $Audit->auditplan->status >= $Audit->auditplan->arrEnumStatus['review_in_progress'] && $Audit->auditplan->reviewer!== null && $Audit->auditplan->reviewer->reviewer_id == $userid && Yii::$app->userrole->hasRights(['audit_review'])){
						$canChangeMaterialComp = 1;
					}
				}


				if($Audit !== null && ($Audit->status == $Audit->arrEnumStatus['rejected']  
				|| $Audit->status == $Audit->arrEnumStatus['nc_overdue_failed']
				|| $Audit->status == $Audit->arrEnumStatus['nc_overdue_waiting_for_review']
