...
class SiteController  extends \yii\rest\Controller
{
	...
	public function actionTestapi(){
		...
		if(isset($data['data'])){
			$imagedata = $data['data'];
			//return $imagedata;
			$company_files = Yii::$app->params['company_files'];
			$output_file = 'testf2.pdf';
			file_put_contents($company_files.$output_file, file_get_contents($imagedata));
			//file_put_contents($company_files.'testrrrr2.jpg', $imagedata);
			//$this->base64_to_jpeg($imagedata,'testrfinal.jpg');
		}
		$datar = [];
		$datar['test'] ='tes na';
        return $datar;
	...
	private function base64_to_jpeg($base64_string, $output_file) {
		...
		$ifp = fopen( $company_files.$output_file, 'wb' ); 
