 ...
 */
class SubTopicController extends \yii\rest\Controller
{
    /**
     * {@inheritdoc}
     */
    ...
    {
		if(!Yii::$app->userrole->hasRights(array('sub_topic_master')))
		{
			return false;
		}
		$post = yii::$app->request->post();
		$date_format = Yii::$app->globalfuns->getSettings('date_format');
		$model = SubTopic::find()->where(['<>','status',2]);
		...
		return ['bsectors'=>$SubTopic_list,'total'=>$totalCount];
    }
	...
	public function actionGetSubTopic()
    {
		$post = yii::$app->request->post();
		$date_format = Yii::$app->globalfuns->getSettings('date_format');
		$model = SubTopic::find()->where(['status'=>0]);
		...
		return ['subtopics'=>$SubTopic_list];
	}
	...
	{
		if(!Yii::$app->userrole->hasRights(array('add_sub_topic')))
		{
			return false;
		}
		$model = new SubTopic();		
		...
		{		
			...
			{
				$responsedata=array('status'=>1,'message'=>'Sub Topic has been created successfully');	
			}
			else
			{
    ...
    {
		if(!Yii::$app->userrole->hasRights(array('edit_sub_topic')))
		{
			return false;
		}
		$data = Yii::$app->request->post();
		...
		if ($data) 
		{
           	$model = SubTopic::find()->where(['id' => $data['id']])->one();
			if ($model !== null)
			{
				$model->name=$data['name'];
				$model->description=$data['description'];
				...
				if($model->validate() && $model->save())
				{
					$responsedata=array('status'=>1,'message'=>'Sub Topic has been updated successfully');
				}
				else
				{
					$responsedata=array('status'=>0,'message'=>$model->errors);
	...
	public function actionCommonUpdate()
	{
		...
		{
			$status = $data['status'];			
			if(!Yii::$app->userrole->canDoCommonUpdate($status,'sub_topic'))
			{
				return false;
			}	

			$id=$data['id'];
           	$model = SubTopic::find()->where(['id' => $id])->one();
			if ($model !== null)
			{
				...
				if($model->validate() && $model->save())
				{
					$msg='';
					if($model->status==0){
						$msg='Sub Topic has been activated successfully';
					}elseif($model->status==1){
						$msg='Sub Topic has been deactivated successfully';
						...
						$msg='Sub Topic has been deleted successfully';
					}
