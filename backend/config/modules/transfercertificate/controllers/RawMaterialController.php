			...
			if ($model !== null)
			{
				...
				if($data['type']=="view")
				{
					...
					if(count($usedweight)>0)
					{
						...
						{ 	
							$TCRequestObj = $value->tcproduct->request;

							$productarr = array();
							$productarr['tc_request_product_id'] = $value->tc_request_product_id;							
							$productarr['tc_nos'] = ($TCRequestObj->arrEnumStatus['approved']==$TCRequestObj->status ? $TCRequestObj->tc_number : 'TEMP'.$TCRequestObj->id);
			...
			{	
				...
				if(is_array($data['products']) && count($data['products'])>0)
				{
					...
					{ 
						...
						$newproduct = 0;
						$new_gross_weight = $value['gross_weight'];
						$new_net_weight = $value['net_weight'];
						$new_certified_weight = $value['certified_weight'];
						if(isset($value['raw_material_product_id']) && $value['raw_material_product_id']!=''){
							...
							if($rawmaterialproduct === null){
								...
								//$rawmaterialproduct0>tcusedweight
								
								
								if($value['net_weight']==0){
									$product_total_used_weight = $rawmaterialproduct->total_used_weight !=''? $rawmaterialproduct->total_used_weight:0;
									$new_gross_weight = $new_gross_weight<$product_total_used_weight?$product_total_used_weight:$new_gross_weight;
									$new_net_weight = $product_total_used_weight;
									$new_certified_weight = $product_total_used_weight;
								}
						...
						$rawmaterialproduct->gross_weight = $new_gross_weight;
						$rawmaterialproduct->actual_net_weight = $new_net_weight;
						if($newproduct){
						...
						if($data['is_certified']==1)
						{
							$rawmaterialproduct->certified_weight = $new_certified_weight;
						}else{
