...
class RequestController extends \yii\rest\Controller
{
		...
		if($post)
		{
			...
			if(count($model)>0)
			{
				...
				{	
					...
					$unitName = $modelData->applicationunit?$modelData->applicationunit->name:'';
					if($modelData->applicationunit && $modelData->applicationunit->unit_type==1)
					{
						$unitName = $modelData->applicationaddress?$modelData->applicationaddress->unit_name:"";
					}
					$data['unit_id_label']=$unitName;
					$data['buyer_id_label']=$modelData->buyer?$modelData->buyer->name:'';
					...
					$data['oss_label'] = $modelData->application?$usermodel->ossnumberdetail($modelData->application->franchise_id):'';
					
					$customeroffernumber = $modelData->application?$modelData->application->customer->customer_number:'';
				...
				{	
					...
					if(count($diffresult)>0 || ($existing_unitid!='' && $existing_unitid!=$model->unit_id) || ($existing_appid!='' && $existing_appid!=$model->app_id) ){
						...
						if(count($TcRequestProductModel)>0)
						{
							...
							//reviewer_declaration_update_status
							//oss_declaration_update_status
							
						}
						if($model->status == $model->arrEnumStatus['review_in_process']){
							$model->reviewer_declaration_update_status = 0;
						}else if($model->status == $model->arrEnumStatus['pending_with_customer'] || $model->status == $model->arrEnumStatus['waiting_for_osp_review'] || $model->status == $model->arrEnumStatus['pending_with_osp'] ){
							$model->reviewer_declaration_update_status = 0;
							$model->oss_declaration_update_status = 0;								
						}
						$model->overall_input_status = 0;
						$model->save();
					}
	...
	*/
	...
	public function actionGetStatus()
	{
		...
		{
			$Request = new Request();
			if($data['status']==  $Request->arrEnumStatus['review_in_process'] && false)
			{
				$model = new RequestReviewerComment();
			}
			else
			{
print_r("fooo");				$model = new RequestFranchiseComment();
	...
	public function actionDeleteproductdata()
	{
		...
		{			
			...
			if($TcRequestProductModel!== null)
			{
				$this->updateProductWeightToRequest($TcRequestProductModel->tc_request_id);
			}
			//
