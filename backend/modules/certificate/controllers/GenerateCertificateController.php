	...
	public function actionCertificateList()
    {
		...
		if($resource_access != 1){
			...
			if($user_type== 1 && ! in_array('certification_management',$rules) && ! in_array('audit_management',$rules) ){
				...
				$model = $model->andWhere(['bran.user_id'=>$userid])->andWhere(['appbrand.status'=>$modelApplication->arrBrandEnumStatus['approved'],'bran.status'=>$brand_mod->arrEnumStatus['active'],'t.certificate_status'=>$modelCertificate->arrEnumCertificateStatus['valid']]);
				...
				$model = $model->andWhere(['bg.user_id'=>$userid])->andWhere(['appbrand.status'=>$modelApplication->arrBrandEnumStatus['approved'],'bran.status'=>$brand_mod->arrEnumStatus['active'],'t.certificate_status'=>$modelCertificate->arrEnumCertificateStatus['valid']]); 
			}	
		}	
