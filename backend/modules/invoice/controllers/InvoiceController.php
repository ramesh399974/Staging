	...
	public function actionIndex()
    {
		...
		if(is_array($post) && count($post)>0 && isset($post['page']) && isset($post['pageSize']))
		{
			...
			if(isset($post['searchTerm']))
			{
				...
				if($invoicetype==2){
					$countmodel->select(' Sum(Case When t.status="5" Then total_payable_amount Else 0 End) paid_amount, SUM(Case When t.status="4" or t.status="6"  Then total_payable_amount Else 0 End) as unpaid_amount ');
				}
				else{
		...
		{	
			...
			if($offermodel!==null)
			{
				...
				if(count($invoicetax)>0)
				{
					...
					$gst=array();
					foreach($invoicetax as $i){
					
						$gst[]=$i->tax_name;
					}
					$offerdata['samtx']= implode(", ",$gst);
				}				
		...
		$html='<style>
			...
			<table cellpadding="0" cellspacing="0" border="0" class="reportDetailLayout" width="100%">
				...
				<tr>
					...
					'.$taxN.' No.&nbsp;&nbsp;'.($arrFranchiseDetails['gst_no'] ? $arrFranchiseDetails['gst_no'] : '-').'
					</td>
				</tr>			
			</table>			
				...
				<tr>	
					<td colspan="4" class="productDetails">
						...
						<table cellpadding="0" cellspacing="0" border="0" class="reportDetailLayout">
							...
							<tr>
								<td style="text-align:left;padding-bottom:5px;" valign="middle" class="reportDetailLayoutInner">'.$taxN.' No.</td>
								<td style="text-align:left;padding-bottom:5px;" valign="middle" class="reportDetailLayoutInner">:</td>
