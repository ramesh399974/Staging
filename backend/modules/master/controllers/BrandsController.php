...
class BrandsController extends \yii\rest\Controller
{
	...
	*/
	...
	public function actionGetConsent()
	{
		...
		if($data){
			$brand_mod = new Brand();
			$audconbrmod = AuditBrandConsent::find()->alias('abc');
			$audconbrmod = $audconbrmod->join('inner join','tbl_brands as br','br.id=abc.brand_id')->where(['abc.app_id'=>$data['app_id'],'abc.unit_id'=>$data['unit_id'],'br.status'=>$brand_mod->arrEnumStatus['active']])->all();
			$brdata= array();
			$brndname = array();
