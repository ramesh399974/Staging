	...
	*/
	...
	public function actionGetUsers()
	{
		...
		if ($data['type'] ==100){
			...
			$brandmod = Brand:: find()->where(['status'=>$brand_mod->arrEnumStatus['active']])->all();
			...
			$brandmod = ApplicationBrands:: find()->alias('ab');
			$brandmod = $brandmod->join('inner join','tbl_brands as br','br.id=ab.brand_id')->where(['ab.app_id'=>$data['id'],'br.status'=>$brand_mod->arrEnumStatus['active']])->all(); 
