	...
	*/
				...
				if($offerlistmodel->validate() && $offerlistmodel->save())
				{
					...
					if (($modelApplication = Application::findOne($offermodel->app_id)) !== null) 
					{
						...
						if($applicationMandayCostObj !== null)
						{
							...
							if($modelApplication->applicationaddress->state_id == $modelApplication->franchise->usercompanyinfo->company_state_id)
							{
							...
							if($franchisesamestate){
								...
								$applicationMandayCostTaxObj = $applicationMandayCostObj->mandaycosttaxotherstate;
							}
				...
				<br>
					...
					<table cellpadding="0" cellspacing="0" border="0" width="100%" class="reportDetailLayout" style="margin-top:50px;text-align:center;border:none;">					
						<tr style="margin-top:20px;text-align:center;">							
							<td class="reportDetailLayoutInner" style="text-align:left;border:none;">Signature of Authorized Person: .................................</td>							
							<td class="reportDetailLayoutInner" style="text-align:center;border:none;">Date: .................................</td>
						</tr>
						<tr style="margin-top:20px;text-align:center;">
							 <td class="reportDetailLayoutInner" style="text-align:center;border:none;">' .$appmodel->contactname.'</td>
				  		</tr>
					</table>
				</div>';
			}
		...
		{	
				...
				{	
					foreach($model->applicationunit as $units)
					{
						...
						*/
						...
						$unit_man_day_cost = $units->mandaycost?$units->mandaycost->man_day_cost:0;
						$appunitmanday->manday_cost=$unit_man_day_cost;
						$appunitmanday->unit_manday_cost=($finalmanday*$unit_man_day_cost);
			...
			if(is_array($model->applicationunit) && count($model->applicationunit)>0)
			{
				foreach($model->applicationunit as $units)
				{
					...
					$appUnitMandayCostArr[$units->id] = $units->mandaycost?$units->mandaycost->man_day_cost:0;
			...
			*/
			...
			if ($model !== null)
			{
				...
				//echo $model->applicationaddress->state_id.' == '.$model->franchise->usercompanyinfo->company_state_id;
				$franchisesamestate = 0;
				if($model->applicationaddress->state_id == $model->franchise->usercompanyinfo->company_state_id){
					$franchisesamestate = 1;
				}
				...
				if($appmdc!==null)
				{
					...
					}elseif($mandaycosttaxobj!==null){
						$mdctaxArr[]=$mandaycosttaxobj? $mandaycosttaxobj->tax_name : '';
						$mdctaxpercentArr[]=$mandaycosttaxobj?$mandaycosttaxobj->tax_percentage:'0';	
					}
				}
	...
	public function actionViewOffer()
	{
		...
		if($postdata){
			...
			$data = $getdata;
			if(isset($data['id']) && $data['id'] !='' && $data['id'] >0 && $data['offer_id']==''){
				$OfferDataModel = Offer::find()->where(['app_id'=>$data['id']])->one();
				if($OfferDataModel !== null){
					$data['offer_id'] = $OfferDataModel->id;
				}
			}
			//echo $data['id']; die;
		}
	...
	public function actionValidateAuditReport()
	{
		...
		if($post)
		{
			...
			*/
			...
			$clientformstatus = true;//Yii::$app->globalfuns->getReportsAccessible($chkdata);
			if($clientformstatus)
			{
	...
	public function actionGetreportlist(){
		...
		*/
		...
		print_r($offermodel->enumStatus['finalized']);
