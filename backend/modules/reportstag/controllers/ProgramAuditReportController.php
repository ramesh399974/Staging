...
/**
 * ProgramAuditReportController implements the CRUD actions for Product model.
 */
class ProgramAuditReportController extends \yii\rest\Controller
{
	...
	public function actionIndex()
    {
		...
		if(count($model)>0)
		{
			if($post['type']!='submit')
			{
				$arrHeaderLabel=array('S.No','Customer Number','Company Name','Country','OSS','Standards','Type','Unit Name','Unit Address','Auditor(s)','Audit Date(s)');
				...
				$sheet = $spreadsheet->getActiveSheet()->setTitle("Audit Report");
			...
			foreach($model as $offer)
			{
				...
				$data['audit_type']=$offer->audittypeArr[$offer->audit_type];
				...
				*/
				...
				//$sc_name_address=$application->applicationunitall;
				$sc_name_address=$offer->auditplan->auditplanunit;
