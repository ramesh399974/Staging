...
@Component({
  selector: 'app-application-product-list',
  templateUrl: './application-product-list.component.html',
  styleUrls: ['./application-product-list.component.scss']
})
export class ApplicationProductListComponent implements OnInit {
  ...
  ngOnInit() 
  {
		...
		/*
		this.enquiry.getProductDetails(this.id).
		subscribe(res => {
  ...
  getProducts(id)
  {
		this.enquiry.getProductDetails({id:id,standard_id:this.standard_id}).subscribe(res => {
			this.applicationData = res;
  ...
  */
  ...
  updateMaterialComposition()
  {
	  ...
	  if(this.productmaterial_error=='')
	  {
		   ...
		   this.enquiry.updateProductMaterialComposition({product_id:this.productEntry.autoid,productmateriallist:productMaterialList})
		  .pipe(first())
		  .subscribe(res => {
