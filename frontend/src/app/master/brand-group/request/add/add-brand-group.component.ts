...
@Component({
  selector: 'app-add-brand-group',
  templateUrl: './add-brand-group.component.html',
  styleUrls: ['./add-brand-group.component.scss']
})
export class AddBrandGroupComponent implements OnInit {

  title = 'Add Brand Group';
  btnLabel = 'Save';
  countryList: Country[];
  stateList: State[];
  ...
  onSubmit() {
    ...
    if (this.form.valid) {
      ...
      this.brandService.addBrandGroup(formvalue)
        .pipe(
          first()
        )
        .subscribe(res => {
