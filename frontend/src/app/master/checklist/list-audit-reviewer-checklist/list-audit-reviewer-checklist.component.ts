...
import {AuditReviewerChecklistListService} from '@app/services/master/checklist/audit-reviewer-checklist-list.service';
...
@Component({
  selector: 'app-list-audit-reviewer-checklist',
  templateUrl: './list-audit-reviewer-checklist.component.html',
  styleUrls: ['./list-audit-reviewer-checklist.component.scss'],
   providers: [AuditReviewerChecklistListService]
})
export class ListAuditReviewerChecklistComponent {

  title='Certificate Reviewer Review Checklist';
  auditreviewerchecklists$: Observable<AuditReviewerChecklist[]>;
  ...
  constructor(public service: AuditReviewerChecklistListService,private modalService: NgbModal,private errorSummary: ErrorSummaryService, private authservice:AuthenticationService,private standardservice: StandardService) {
    this.auditreviewerchecklists$ = service.auditreviewerchecklists$;
    ...
    this.authservice.currentUser.subscribe(x => {
      if(x){
        ...
        {
          if(this.userdetails.rules.includes('edit_audit_reviewer_review_checklist')  ){
            this.canEditData = true;
          }
          if(this.userdetails.rules.includes('delete_audit_reviewer_review_checklist') ){
            this.canDeleteData = true;
          }
          if(this.userdetails.rules.includes('add_audit_reviewer_review_checklist')  ){
            this.canAddData = true;
          }
          if(this.userdetails.rules.includes('activate_audit_reviewer_review_checklist')  ){
            this.canActivateData = true;
          }
          if(this.userdetails.rules.includes('deactivate_audit_reviewer_review_checklist')  ){
