...
@Component({
  ...
  templateUrl: '../add-sub-topic/add-sub-topic.component.html',
...
export class EditSubTopicComponent implements OnInit {
  ...
  constructor(private activatedRoute:ActivatedRoute,private router: Router,private fb:FormBuilder,private SubTopicService: SubTopicService,private errorSummary: ErrorSummaryService) { }
  ...
  ngOnInit() {
    ...
    this.SubTopicService.getSubTopic(this.id).pipe(first())
    .subscribe(res => {
  ...
  onSubmit(){
    ...
    if (this.form.valid) {
        this.loading = true;      
		this.SubTopicService.updateData(this.form.value).pipe(first()
      ).subscribe(res => {
