...
import { InvoiceReportService } from '@app/services/report/invoice-report.service';
...
export class InvoiceReportComponent implements OnInit {
  ...
  constructor(private modalService: NgbModal, private userservice: UserService,private router: Router,private fb:FormBuilder,private standardservice: StandardService, private authservice:AuthenticationService,private errorSummary: ErrorSummaryService,public service: InvoiceReportService) { }
  ...
  ngOnInit() {
    ...
    .subscribe(res => {
      this.statuslist  = res.filterpaymentOptions; 
    });
  ...
  onSubmit(type,filename='')
  {
    ...
    this.f.invoicetype.markAsTouched();
    this.f.status_id.markAsTouched();
    ...
    let invoicetype = this.form.get('invoicetype').value;
    let status_id = this.form.get('status_id').value;
  
    if((from_date=='' || from_date===null) && (to_date=='' || to_date===null) && standard_id=='' && oss_id=='' && status_id=='')
    {
    ...
    if (this.fieldErrors=='' && this.from_dateErr=='' && this.to_dateErr=='') 
    {
      ...
      let expobject:any={from_date:from_date,to_date:to_date,standard_id:standard_id,oss_id:oss_id,status_id:status_id,type:type,invoicetype:invoicetype};
      if(1)
      {
        if(type=='submit')
        {
