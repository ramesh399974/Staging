...
import { PersonnelReportService } from '@app/services/report/personnel-report.service';
...
@Component({
  ...
  styleUrls: ['./personnel-report.component.scss']
})
export class PersonnelReportComponent implements OnInit {
  ...
  constructor(private modalService: NgbModal, private userservice: UserService,private router: Router,private fb:FormBuilder,private standardservice: StandardService, private authservice:AuthenticationService,private errorSummary: ErrorSummaryService,public service: PersonnelReportService) { }
  ...
  ngOnInit() {
    ...
    this.service.getAuditorTeList().subscribe(res => {
      this.auditorteList = res['rolelist'];     
    });
  ...
  getSelectedAuditorValue(val)
  {
    return this.auditorteList.find(x=> x.id==val).name;    
  }
  ...
  onSubmit(type,filename='')
  {
    ...
    let usertype = this.form.get('usertype').value;
  
    if((from_date=='' || from_date===null) && (to_date=='' || to_date===null) && standard_id=='' && oss_id=='' && usertype=='')
    {
    ...
    if (this.fieldErrors=='' && this.from_dateErr=='' && this.to_dateErr=='') 
    {
      ...
      let expobject:any={from_date:from_date,to_date:to_date,standard_id:standard_id,oss_id:oss_id,type:type,usertype:usertype};
      if(1)
      {
        if(type=='submit')
        {
