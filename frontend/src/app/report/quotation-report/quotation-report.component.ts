...
import { QuotationReportService } from '@app/services/report/quotation-report.service';
...
export class QuotationReportComponent implements OnInit {
  ...
  constructor(private modalService: NgbModal, private userservice: UserService,private router: Router,private fb:FormBuilder,private standardservice: StandardService, private authservice:AuthenticationService,private errorSummary: ErrorSummaryService,public service: QuotationReportService) { }
  ...
  onSubmit(type,filename='')
  {
    ...
    if (this.fieldErrors=='' && this.from_dateErr=='' && this.to_dateErr=='') 
    {
      ...
      let expobject:any={from_date:from_date,to_date:to_date,standard_id:standard_id,oss_id:oss_id,status_id:status_id,type:type};
      if(1)
      {
        if(type=='submit')
        {
