...
@Component({
  selector: 'app-tc-report',
  templateUrl: './tc-report.component.html',
  styleUrls: ['./tc-report.component.scss']
})
export class TcReportComponent implements OnInit {
  ...
  constructor(private modalService: NgbModal, private userservice: UserService,private router: Router,private fb:FormBuilder,private standardservice: StandardService, private authservice:AuthenticationService,private errorSummary: ErrorSummaryService,public service: TcReportService) { }
  ...
  onSubmit(type,filename='')
  {
    ...
    if((from_date=='' || from_date===null) && (to_date=='' || to_date===null) && standard_id=='' && oss_id=='' && app_id=='')
    {
    ...
    if (this.fieldErrors=='' && this.from_dateErr=='' && this.to_dateErr=='') 
    {
      ...
      let expobject:any={from_date:from_date,to_date:to_date,standard_id:standard_id,oss_id:oss_id,type:type,app_id:app_id};
      if(1)
      {
        if(type=='submit')
        {
