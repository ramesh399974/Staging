...
import {CustomerClientlogoRequestList} from '@app/models/application/customer-clientlogo-request-list';

interface SearchResult {
  requests: CustomerClientlogoRequestList[];
  total: number;
}
interface State {
  page: number;
...
function sort(requests: CustomerClientlogoRequestList[], column: string, direction: string): CustomerClientlogoRequestList[] {
  //console.log('234324');
  if (direction === '') {
    return requests;
  } else {
...
function matches(auditreviewerchecklist: CustomerClientlogoRequestList, term: string, pipe: PipeTransform) {
  return auditreviewerchecklist.company_name.toLowerCase().includes(term.toLowerCase());  
}
...
export class CustomerClientlogoRequestListService {
  private _loading$ = new BehaviorSubject<boolean>(true);
  private _search$ = new Subject<void>();
  private _requests$ = new BehaviorSubject<CustomerClientlogoRequestList[]>([]);
  ...
  private _search(): Observable<SearchResult> {
    ...
    return this.http.post<SearchResult>(`${environment.apiUrl}/application/clientlogo-request/index`,{category:this.category,page,pageSize,searchTerm,sortColumn,standardFilter,sortDirection}).pipe(
        map(result => {
  ...
  commonActionData(data): Observable<any>{
    return this.http.post<any>(`${environment.apiUrl}/application/clientlogo-request/common-update`,data);
  } 
}
