...
export class CertificateGenerationListService {
  ...
  private _search(): Observable<SearchResult> {
    ...
    const {sortColumn, sortDirection, statusFilter, pageSize, page, searchTerm, standardFilter, countryFilter} = this._state;
    ...
    */
    ...
    return this.http.post<SearchResult>(`${environment.apiUrl}/certificate/generate-certificate/certificate-generation-list`,{type:'audit',page,pageSize,searchTerm,statusFilter,sortColumn,sortDirection,standardFilter,countryFilter}).pipe(
        map(result => {
  ...
  public customSearch(){
    this._listauditplan$.next([]);
    this._total$.next(0);
    this._loading$.next(true);
    this._search$.next();
  }

  CertificateStatus(): Observable<any>{
    return this.http.post<any>(`${environment.apiUrl}/certificate/generate-certificate/certificate-status`,{});
  }

  downloadFile(data){
    return this.http.post(`${environment.apiUrl}/certificate/generate-certificate/download-file`,data,
  ...
  addCB(data)
  {
    return this.http.post<any>(`${environment.apiUrl}/certificate/generate-certificate/save-cb`,data);
  }
