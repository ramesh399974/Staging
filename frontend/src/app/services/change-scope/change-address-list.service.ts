...
function sort(changeaddresses: Changeaddress[], column: string, direction: string): Changeaddress[] {
  if (direction === '') {
    return changeaddresses;
  } else {
    return [...changeaddresses].sort((a, b) => {
...
function matches(changeaddresses: Changeaddress, term: string, pipe: PipeTransform) {

  return changeaddresses.company_name.toLowerCase().includes(term.toLowerCase());
}
...
export class ChangeAddressListService {
  private _loading$ = new BehaviorSubject<boolean>(true);
  private _search$ = new Subject<void>();
  private _changeaddresses$ = new BehaviorSubject<Changeaddress[]>([]);
  ...
  constructor( private activatedRoute:ActivatedRoute,private http:HttpClient,public errorSummary: ErrorSummaryService) {
    ...
    this._search$.pipe(
      ...
      this._changeaddresses$.next(result.changeaddresses);
  ...
  get changeaddresses$() { return this._changeaddresses$.asObservable(); }
  ...
  private _search(): Observable<SearchResult> {
    ...
    return this.http.post<SearchResult>(`${environment.apiUrl}/changescope/change-address/index`,{type:this.type,page,pageSize,searchTerm,sortColumn,sortDirection,statusFilter,typeFilter}).pipe(
        map(result => {
          return {changeaddresses:result.changeaddresses,total:result.total};
        })
    );
  ...
  commonActionData(data): Observable<any>{
    return this.http.post<any>(`${environment.apiUrl}/changescope/change-address/common-update`,data);
  } 
  // ------ New Code End Here ---------
