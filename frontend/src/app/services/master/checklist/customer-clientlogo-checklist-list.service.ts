...
interface SearchResult {
  informationchecklists: CustomerClientlogoChecklist[];
  total: number;
}
interface State {
  page: number;
...
function sort(informationchecklists: CustomerClientlogoChecklist[], column: string, direction: string): CustomerClientlogoChecklist[] {
  //console.log('234324');
  if (direction === '') {
...
function matches(auditreviewerchecklist: CustomerClientlogoChecklist, term: string, pipe: PipeTransform) {
...
export class CustomerClientlogoChecklistListService {
  private _loading$ = new BehaviorSubject<boolean>(true);
  private _search$ = new Subject<void>();
  private _informationchecklists$ = new BehaviorSubject<CustomerClientlogoChecklist[]>([]);
  ...
  private _search(): Observable<SearchResult> {
    ...
    return this.http.post<SearchResult>(`${environment.apiUrl}/master/customer-clientlogo-checklist/index`,{category:this.category,page,pageSize,searchTerm,sortColumn,standardFilter,sortDirection}).pipe(
        map(result => {
  ...
  commonActionData(data): Observable<any>{
    return this.http.post<any>(`${environment.apiUrl}/master/customer-clientlogo-checklist/common-update`,data);
  } 
}
