...
interface SearchResult {
  informationchecklists: HqClientlogoChecklist[];
  total: number;
}
interface State {
  page: number;
...
function sort(informationchecklists: HqClientlogoChecklist[], column: string, direction: string): HqClientlogoChecklist[] {
  //console.log('234324');
  if (direction === '') {
...
function matches(auditreviewerchecklist: HqClientlogoChecklist, term: string, pipe: PipeTransform) {
...
export class HqClientlogoChecklistListService {
  private _loading$ = new BehaviorSubject<boolean>(true);
  private _search$ = new Subject<void>();
  private _informationchecklists$ = new BehaviorSubject<HqClientlogoChecklist[]>([]);
  ...
  private _search(): Observable<SearchResult> {
    ...
    return this.http.post<SearchResult>(`${environment.apiUrl}/master/hq-clientlogo-checklist/index`,{category:this.category,page,pageSize,searchTerm,sortColumn,standardFilter,sortDirection}).pipe(
        map(result => {
  ...
  commonActionData(data): Observable<any>{
    return this.http.post<any>(`${environment.apiUrl}/master/hq-clientlogo-checklist/common-update`,data);
  } 
}
