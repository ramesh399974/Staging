...
function sort(buyer: Buyer[], column: string, direction: string): Buyer[] {
  if (direction === '') {
    return buyer;
  } else {
    return [...buyer].sort((a, b) => {
...
function matches(buyer: Buyer, term: string, pipe: PipeTransform) {

  return buyer.name.toLowerCase().includes(term.toLowerCase());
}
...
export class BuyerListService {
  ...
  private _buyer$ = new BehaviorSubject<Buyer[]>([]);
  ...
  get buyer$() { return this._buyer$.asObservable(); }
  ...
  private _search(): Observable<SearchResult> {
    ...
    */
    ...
    return this.http.post<SearchResult>(`${environment.apiUrl}/transfercertificate/buyer/index`,{type:this.type,page,pageSize,statusFilter,searchTerm,sortColumn,sortDirection}).pipe(
        map(result => {
          return {buyer:result.buyer,total:result.total};
        })
    );
  ...
  getData(data){
    return this.http.post<any>(`${environment.apiUrl}/transfercertificate/buyer/get-data`, data);
  }

  addData(data){
    return this.http.post<any>(`${environment.apiUrl}/transfercertificate/buyer/create`, data);
  }
 
  deleteData(data){
    return this.http.post<any>(`${environment.apiUrl}/transfercertificate/buyer/deletedata`, data);
  }
  
  downloadFile(data){
    return this.http.post(`${environment.apiUrl}/transfercertificate/buyer/downloadfile`,data,
      {responseType:'arraybuffer'}
    );
  }

  getAccessData(data:any={}){
    return this.http.post<any>(`${environment.apiUrl}/transfercertificate/buyer/getaccessdata`, data);
  }
  ...
  commonActionData(data): Observable<any>{
    return this.http.post<any>(`${environment.apiUrl}/transfercertificate/buyer/common-update`,data);
  } 
  // ------ New Code End Here ---------
