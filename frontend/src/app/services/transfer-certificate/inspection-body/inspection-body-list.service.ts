...
function sort(inspectionbody: InspectionBody[], column: string, direction: string): InspectionBody[] {
  if (direction === '') {
    return inspectionbody;
  } else {
    return [...inspectionbody].sort((a, b) => {
...
function matches(inspectionbody: InspectionBody, term: string, pipe: PipeTransform) {

  return inspectionbody.name.toLowerCase().includes(term.toLowerCase());
}
...
export class InspectionBodyListService {
  private _loading$ = new BehaviorSubject<boolean>(true);
  private _search$ = new Subject<void>();
  private _inspectionbody$ = new BehaviorSubject<InspectionBody[]>([]);
  ...
  constructor( private activatedRoute:ActivatedRoute,private http:HttpClient,public errorSummary: ErrorSummaryService) {
    ...
    this._search$.pipe(
      ...
      this._inspectionbody$.next(result.inspectionbody);
  ...
  get inspectionbody$() { return this._inspectionbody$.asObservable(); }
  ...
  private _search(): Observable<SearchResult> {
    ...
    */
    ...
    return this.http.post<SearchResult>(`${environment.apiUrl}/transfercertificate/inspection-body/index`,{type:this.type,page,pageSize,statusFilter,searchTerm,sortColumn,sortDirection}).pipe(
        map(result => {
          return {inspectionbody:result.inspectionbody,total:result.total};
        })
    );
  ...
  getData(){
    return this.http.post<any>(`${environment.apiUrl}/transfercertificate/inspection-body/get-data`, {});
  }

  addData(data){
    return this.http.post<any>(`${environment.apiUrl}/transfercertificate/inspection-body/create`, data);
  }
 
  deleteData(data){
    return this.http.post<any>(`${environment.apiUrl}/transfercertificate/inspection-body/deletedata`, data);
  }
  
  downloadFile(data){
    return this.http.post(`${environment.apiUrl}/transfercertificate/inspection-body/downloadfile`,data,
  ...
  commonActionData(data): Observable<any>{
    return this.http.post<any>(`${environment.apiUrl}/transfercertificate/inspection-body/common-update`,data);
  } 
  // ------ New Code End Here ---------
