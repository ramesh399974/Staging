...
interface SearchResult {
  standardLabelGrade: StandardLabelGrade[];
  total: number; 
}
interface State {
  page: number;
...
function sort(standardLabelGrade: StandardLabelGrade[], column: string, direction: string): StandardLabelGrade[] {
  if (direction === '') {
    return standardLabelGrade;
  } else {
    return [...standardLabelGrade].sort((a, b) => {
...
export class StandardLabelGradeListService {
  ...
  private _standardLabelGrade$ = new BehaviorSubject<StandardLabelGrade[]>([]);
  ...
  constructor( private activatedRoute:ActivatedRoute,private http:HttpClient,public errorSummary: ErrorSummaryService) {
    ...
    this._search$.pipe(
      ...
      this._standardLabelGrade$.next(result.standardLabelGrade);
  ...
  get standardLabelGrade$() { return this._standardLabelGrade$.asObservable(); }
  ...
  private _search(): Observable<SearchResult> {
    ...
    const {sortColumn, sortDirection, pageSize,statusFilter,standardFilter, page, searchTerm} = this._state;
	/*
    ...
    */
    ...
    return this.http.post<SearchResult>(`${environment.apiUrl}/transfercertificate/tc-standard-label-grade/index`,{type:this.type,page,pageSize,statusFilter,standardFilter,searchTerm,sortColumn,sortDirection}).pipe(
        map(result => {
          return {standardLabelGrade:result.standardLabelGrade,total:result.total};
        })
    );
  ...
  getData(){
    return this.http.post<any>(`${environment.apiUrl}/transfercertificate/tc-standard-label-grade/get-data`, {});
  }

  addData(data){
    return this.http.post<any>(`${environment.apiUrl}/transfercertificate/tc-standard-label-grade/create`, data);
  }
 
  deleteData(data){
    return this.http.post<any>(`${environment.apiUrl}/transfercertificate/tc-standard-label-grade/deletedata`, data);
  }
  
  downloadFile(data){
    return this.http.post(`${environment.apiUrl}/transfercertificate/tc-standard-label-grade/downloadfile`,data,
  ...
  commonActionData(data): Observable<any>{
    return this.http.post<any>(`${environment.apiUrl}/transfercertificate/tc-standard-label-grade/common-update`,data);
  } 
  // ------ New Code End Here ---------
